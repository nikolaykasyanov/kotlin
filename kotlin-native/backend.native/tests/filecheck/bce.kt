...
fun forEachIndicies() {
    ...
    for (i in array.indices) {
        // CHECK: call void @Kotlin_Array_set_without_BoundCheck
        array[i] = 6
    }
}
...
fun forUntilWithStep() {
    ...
    for (i in 0 until array.size step 2) {
        // CHECK: call void @Kotlin_CharArray_set_without_BoundCheck
        array[i] = '6'
    }
}
// CHECK-LABEL: {{^}}epilogue:
...
fun forDownToWithStep() {
    ...
    for (i in array.size - 1 downTo 0 step 2) {
        // CHECK: call void @Kotlin_IntArray_set_without_BoundCheck
        array[i] = 6U
    }
}
// CHECK-LABEL: {{^}}epilogue:
...
fun forWithIndex() {
    ...
    for ((index, value) in array.withIndex()) {
        // CHECK: call %struct.ObjHeader* @Kotlin_Array_get_without_BoundCheck
        array[index] = 6
    }
}
// CHECK-LABEL: {{^}}epilogue:
...
fun innerLoop() {
    ...
    // CHECK: {{^}}do_while_loop{{.*}}:
    for (i in 0 until array.size) {
        // CHECK-DAG: call %struct.ObjHeader* @Kotlin_Array_get_without_BoundCheck
        array[i] = 7
        // CHECK-DAG: call void @Kotlin_Array_set_without_BoundCheck
        // CHECK-DAG: call void @Kotlin_Array_set_without_BoundCheck
...
fun argsInFunctionCall() {
    ...
    for (i in 0..size1) {
        // CHECK: call %struct.ObjHeader* @Kotlin_Array_get_without_BoundCheck
        // CHECK: call %struct.ObjHeader* @Kotlin_Array_get_without_BoundCheck
        // CHECK: call i32 @"kfun:#foo(kotlin.Int;kotlin.Int){}kotlin.Int"
        foo(array[i], array[i])
    }
}
// CHECK-LABEL: {{^}}epilogue:
